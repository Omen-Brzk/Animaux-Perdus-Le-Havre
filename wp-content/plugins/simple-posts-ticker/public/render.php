...
function spt_render_posts_ticker( $atts ) {
    ...
    $stop = isset($spt_settings['spt_stop_on_hover']) ? $spt_settings['spt_stop_on_hover'] : 'false';
    ...
    $content .= '<div class="spt-content" data-stop="'.$atts['on_stop'].'" data-gap="'.$atts['infinite_scroll'].'" data-speed="'.$atts['speed'].'" style="margin: '.$atts['margin'].';font-size: '.$atts['size'].';">';
    if ( count( $posts ) == 0 ) {
        ...
        foreach ( $posts as $current_post ) {
            ...
            $content .= '<span class="spt-item" style="padding: '.$atts['padding'].';">';
            $content .= '<a class="'.$linkclass.'"'.$no_follow.' style="color: '.$atts['colour'].';" target="'.$atts['target'].'" href="'.apply_filters( 'spt_post_custom_redir_link', get_permalink() ).'">'.apply_filters( 'spt_post_title_prefix', '' ).substr( get_the_title(), 0, apply_filters( 'spt_post_title_length', '150' ) );
